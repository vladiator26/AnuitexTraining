    ...
    public class PrintingEditionRepository : BaseRepository<PrintingEdition>, IPrintingEditionRepository
    {
            ...
            PrintingEdition filter = null, string author = null, string orderField = null, bool descending = false)
        {
            ...
            if (filter != null)
            {
                ...
                printingEditions = _dbSet.Where(item => item.Description.ToLower().Contains(filter.Description.ToLower()));
                printingEditions = _dbSet.Where(item => item.CreationDate.CompareTo(filter.CreationDate) == 0 || filter.CreationDate == default);
                printingEditions = _dbSet.Where(item => item.Currency == filter.Currency || filter.Currency == CurrencyType.None);
                printingEditions = _dbSet.Where(item => item.Price.ToString().Contains(filter.Price.ToString()) || filter.Price == default);
                printingEditions = _dbSet.Where(item => item.Type == filter.Type || filter.Type == PrintingEditionType.None);
                printingEditions = _dbSet.Where(item => item.AuthorInPrintingEdition.Author.Name.ToLower().Contains(author.ToLower()));
            }
